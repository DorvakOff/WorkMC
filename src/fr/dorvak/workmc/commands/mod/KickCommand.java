...
public class KickCommand extends CommandListener {
	...
	public CommandResult perform(Command command, String[] args) {
		...
		kicked.setTitle("Vous avez été expulsé de WorkMc");
		
		if(reason.replace(" ", "").length() > 1) {
			kicked.setDescription("Vous avez été expulsé pour : " + reason);
		}
		...
		if(reason.replace(" ", "").length() > 1) {
			embed.setDescription("Vous avez expulsé " + target.getAsMention() + " pour : " + reason);
		} else {
			embed.setDescription("Vous avez expulsé " + target.getAsMention());
		}
