...
public class MuteCommand extends CommandListener {
	...
	public CommandResult perform(Command command, String[] args) {
		...
		String reason = message.getContentDisplay().replace(WorkMc.getInstance().getPrefixOnDiscord() + "mute ", "").replace("@" + target.getEffectiveName(), "");
		...
		if(reason.replace(" ", "").length() > 1) {
			embed.setDescription(target.getAsMention() + " a été mute par " + user.getAsMention() + " pour : " + reason);
		} else {
